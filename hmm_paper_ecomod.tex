...
\begin{document}
	...
	%TO DO
	% - update the abstract to be more model-focused (perhaps after the major rewrite)
	% - give an introduction that follows another EcoMod paper
	% - Rewrite the model sections, and expand up on CTCRW, HMM, and Kriging
	% - Write Model Evaluation section; look into how to do HMM sensitivity analyses
	
	\begin{abstract}
		Continuous time correlated random walks (CTCRWs) combined with hidden Markov models (HMMs) are a popular way to study animal behavior given movement telemetry time series data. In this paper, we introduce a method to incorporate spatially continuous environmental variables into the HMM framework, by interpolating values at the individuals' predicted positions given by the CTCRW. To demonstrate this method, we use data from a 2018 pond study at the Columbia Environmental Research Center. This study investigated the effect of underwater acoustic deterrents on invasive carp behavior, among other environmental variables. We utilize the ``kriging" interpolation method to incorporate underwater sound intensity as a covariate in the HMM, and linear interpolation to incorporate pond surface temperature. We then investigate the relationship between these variables and the carp's behavioral states.
...
%		Silver carp (\emph{Hypophthalmychthys molitrix}) are one of four primary species of invasive cyprinids in the Mississippi River Basin. They threaten the ecosystems in which they have been introduced by competing with native planktivores and the early life stages of other native fishes. Additionally, silver carp can injure recreational boaters, thus causing negative economic impacts. To mitigate their spread, non-physical barriers such as electrical barriers, bubble curtains, carbon dioxide, and acoustic deterrents have been installed at locks and dams throughout the Mississippi River Basin. In this article, we focus on acoustic deterrents, and investigate their effect on behavioral changes on silver carp, using data from a 2018 pond study performed at the Columbia Environmental Research Center. We use a modeling approach combining continuous-time correlated random walks with a hidden Markov model for two behavioral states. We incorporate interpolated sound-intensity values from the acoustic deterrent as a novel covariate. We also investigate the effect of pond-surface temperature and diel effects on silver carp behavior changes.
	...
	Important factors in animal behavior often vary continuously in space and time; these might include things like food availability, the velocity of wind or water currents, temperature, or the intensity of environmental noise. However, these values can only be sampled at spatially or temporally discrete points which are likely different than the positions in the recorded animal telemetry. In order to associate a value of spatially continuous covariates with each animal position predicted by the CTCRW, we propose the kriging method of spatial interpolation.
