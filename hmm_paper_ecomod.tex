...
\begin{document}
	...
	In this study, we combine a movement modeling framework developed in \cite{Johnson2008, McClintock2012, Michelot2016, Whoriskey2017, McClintock2018} with a kriging process to incorporate spatially continuous covariates. The movement modeling framework involves first fitting a continuous time correlated random walk (CTCRW) to animal telemetry data, which is used to predict temporally regular positions for each individual. With these temporally regular positions, the next step is to fit a Hidden Markov Model (HMM) to the movement tracks. The HMM allows one to predict behavioral states of individuals based on environmental covariates.
